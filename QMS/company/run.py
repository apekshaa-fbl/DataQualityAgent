"""
Company Pipeline — Entry Point

Usage:
    python3 run.py --s3-path s3://fi-firmographics/data_quality/quality_monitoring/company/input/dq_company_04_24_26.csv
...
Pipeline stages (all automatic — no prompts):
    Step 1 — Load CSV from S3, write to BI.RAW.DQMS_COMP_RAW_DATA
    Step 2 — Call Vetric API (200 / 404 / 409 only — all others retried)
    Step 3 — Write BI.RAW.DQMS_COMP_INP_WITH_VETRIC
...
def stage_load_s3(state, bucket, key):
    ...
    from config import normalise_company_slug
    ...
    seen = {}
    for row in rows:
        slug = normalise_company_slug(row.get("inp_linkedin") or "")
        if not slug:
            continue
        if slug in seen:
    ...
    no_slug  = sum(1 for r in rows if not normalise_company_slug(r.get("inp_linkedin") or ""))
...
def stage_write_raw(conn, state):
    ...
    conn.commit()
    print(f"    {n} rows written to BI.RAW.DQMS_COMP_RAW_DATA")


def stage_vetric(state):
    from src.vetric_company import fill_all, retry_400s
    from src.checkpoint import update_stats
    rows = state["_rows"]
    ...
    # fill_all resolves all statuses to 200 or 404 internally.
    # retry_400s is a safety net in case any 400 slipped through.
...
def stage_write_vetric(conn, state):
    ...
    conn.commit()
    print(f"    {n} rows written to BI.RAW.DQMS_COMP_INP_WITH_VETRIC")


def stage_supabase(state):
    from src.supabase_upsert import upsert_company_vetric_rows
    rows = state["_rows"]
    n = upsert_company_vetric_rows(rows)
...
def main():
    parser = argparse.ArgumentParser(description="Company Pipeline")
    parser.add_argument("--s3-path", required=True,
                        help="S3 path to input CSV  e.g. s3://fi-firmographics/.../dq_company_04_24_26.csv")
    ...
    run_auto(state, STAGE_WRITE_RAW,
             "Write BI.RAW.DQMS_COMP_RAW_DATA",
    ...
    run_auto(state, STAGE_WRITE_VETRIC,
             "Write BI.RAW.DQMS_COMP_INP_WITH_VETRIC",
    ...
    run_auto(state, STAGE_COVERAGE_DDL,
             "CREATE coverage monitor table (if not exists)",
    ...
    run_auto(state, STAGE_ACCURACY_DDL,
             "CREATE accuracy checks table (if not exists)",
    ...
    run_auto(state, STAGE_VIEW_DASHBOARD,
             "CREATE OR REPLACE VIEW dqms_comp_dashboard_v",
             run_view_dashboard, conn)

    run_auto(state, STAGE_VIEW_BY_COUNTRY,
             "CREATE OR REPLACE VIEW dqms_comp_overall_run_by_country",
             run_view_by_country, conn)

    run_auto(state, STAGE_VIEW_OVERALL,
             "CREATE OR REPLACE VIEW dqms_comp_overall_run",
